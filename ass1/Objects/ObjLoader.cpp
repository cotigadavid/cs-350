...
std::optional<MeshUniquePtr> ObjLoader::LoadObj(const char *fileLocation) {
	...
	while (true) {
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			newMesh->VertexNormals.push_back(vertexNormal);
			...
			if(fgets(buffer, sizeof(buffer) * sizeof(char), file))
			{
				...
				unsigned int vertexNormalIndex = -1;
				...
				while(token != nullptr) {
					...
					char a, b;
					sscanf(token, "%c%c", &a, &b);
					sscanf(token, "d", &vertexNormalIndex);
					...
					if(count < 3)
					{
						...
						normalIndices.push_back(vertexNormalIndex);
					}
	...
	const glm::vec2 outMinMax = {-2.0f, 2.0f};
	...
	AssembleBuffers(*newMesh);

	return newMesh;
}

void ObjLoader::AssembleBuffers(Mesh& mesh)
{
	int offset = 0;
	for (int i = 0; i < mesh.faceNormals.size(); i++)
	{
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index1].x);
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index1].y);
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index1].z + 1);
		
		mesh.rawVertices.push_back(mesh.faceNormals[i].x);
		mesh.rawVertices.push_back(mesh.faceNormals[i].y);
		mesh.rawVertices.push_back(mesh.faceNormals[i].z);
		
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index2].x);
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index2].y);
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index2].z + 1);
		
		mesh.rawVertices.push_back(mesh.faceNormals[i].x);
		mesh.rawVertices.push_back(mesh.faceNormals[i].y);
		mesh.rawVertices.push_back(mesh.faceNormals[i].z);
		
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index3].x);
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index3].y);
		mesh.rawVertices.push_back(mesh.Vertices[mesh.Faces[i].index3].z + 1);
		
		mesh.rawVertices.push_back(mesh.faceNormals[i].x);
		mesh.rawVertices.push_back(mesh.faceNormals[i].y);
		mesh.rawVertices.push_back(mesh.faceNormals[i].z);

		mesh.rawIndices.push_back(offset);
		mesh.rawIndices.push_back(offset + 1);
		mesh.rawIndices.push_back(offset + 2);
